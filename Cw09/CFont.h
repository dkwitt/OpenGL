...
#ifndef __TEXT_HEADER
#define __TEXT_HEADER
...
void translation(glm::vec3 vec)
    {
        projection = glm::translate( projection, vec);
    };
...
void Reshape_text(){
    ...
    projection = glm::ortho(0.0f, static_cast<float>(Window_Width), 0.0f, static_cast<float>(Window_Height));
    glUseProgram(text_program);
...
// ----------------------------------------------------------------------------------
void InitText(const char* filename, int fontsize)
{
    // Fonts
    strcat(dir, filename);
